...
def generate_plot_data():
    ...
    prob_down_list = []
    profits_list = []
    for days_before_break in xrange(1, 251):
        logging.info("On day : {}".format(days_before_break))
        for prob_down in np.linspace(0.5, 1, 250):
            days_before_break_up_list.append(days_before_break)
            prob_down_list.append(prob_down)
            profits_list.append(calc_profit(250, days_before_break, prob_down))
    return days_before_break_up, prob_down_list, profits_list
